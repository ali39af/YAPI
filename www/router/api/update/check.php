...
if ($api_status == true) {
    ...
    $sql_1 = "SELECT * FROM `update` WHERE `id` = '" . $id . "'";
    $result = mysqli_query($mysql_connection, $sql_1);
    $row = mysqli_fetch_array($result, MYSQLI_ASSOC);
    $count = mysqli_num_rows($result);
    if ($count == 1) {
        http_response_code(202);
        echo '{"status":"202","message":"update exist!","version":"' . $row["version"] . '","news":"' . $row["news"] . '","download":"' . (($row["download"] == '') ? "false" : "true") . '"}';
    } else {
        http_response_code(404);
        echo '{"status":"404","message":"update not found!"}';
    }
} else {
